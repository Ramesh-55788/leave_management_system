...
async function carryForwardLeaves() {
    const currentYear = new Date().getFullYear();
    const previousYear = currentYear - 1;

    const leaveTypes = await leaveTypeRepo.find(); 
    const validTypes = leaveTypes.filter(lt => lt.id !== 4 && lt.id !== 5 && lt.id !== 6 && lt.id !== 7); 
    ...
    for (const type of validTypes) {
        ...
        for (const bal of previousBalances) {
            ...
            if (!existingCurrent) {
                ...
                console.log(`Already exists for user ${bal.userId} and leave type ${type.id}, skipping.`);
            }
        }
    }
...
module.exports = runCarryForwardJob;
