...
const requestLeave = async (
  ...
  const existingLeaves = await leaveRequestRepository.findLeaveOverlappingDates(
    userId,
    new Date(startDate),
    new Date(endDate)
  );
  ...
  if (existingLeaves.length > 0) {
    throw new Error('You already have a leave request with these dates.');
  }
...
const approveLeave = async (requestId) => {
  ...
  if (leaveTypeId === 6) {
    ...
    return { nextStep: 'Auto-approved emergency leave' };
  }
